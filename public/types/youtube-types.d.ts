...
export namespace yt {
  ...
  namespace Search {
    ...
    type SearchServerResponse = {
      info?: VideoInfo | ListInfo;
      type: "video" | "list" | "none";
      errMsg?: string;
    };

    type SearchActionReturn = {
      error: boolean;
      errMsg?: string;
      type?: "video" | "list" | "none";
      data?: VideoInfo | ListInfo;
    };
  }
  // --------------------------- Search End ---------------------------------
  ...
  // ----------------------------- Download Start --------------------------------
  namespace Download {
    type VideoDownloadRequestInfo = {
      videoDetails: ytdl.MoreVideoDetails;
    };

    type VideoQuality = 4320 | 2160 | 1440 | 1080 | 720 | 480 | 360 | 240 | 144;
    type AudioQuality = 160 | 128 | 64 | 48;
    type DownloadQuality = VideoQuality | AudioQuality;

    type ListConfiguration = {
      indicesList: Set<number>;
      total: number;
    };

    type ListDownloadRequestInfo = {
      listInfo: yt.Search.ListInfo;
      indicesList: Set<number>;
    };
  }
  // ------------------------------ Download End ---------------------------------
  ...
  namespace Progress {
    ...
    type ProgressApiResponse = {
      state: "fetch" | "progress" | "error";
      msg: string;
      progress: number;
    };

    type ProgressStateAction = {
      state: BtnStyleClass;
      msg?: string;
      progress?: number;
    };
...
export namespace ytdl {
  ...
  interface downloadOptions extends chooseFormatOptions {
    range?: {
      start?: number;
      end?: number;
    };
